...
def determine_FTR_bid(load_pickle=0):
    ...
    Rowplayer = gv.producers[0]
    Columnplayer = gv.producers[1]
    #FTR_action_space = [-0.1,0.00001,0.9,1,1.1]
    #FTR_action_space = [0.9,1,1.1]
    FTR_action_space = [0.8,1,1.2]
    #FTR_action_space = [1,1.25]
    #FTR_action_space = [0.0001,0.9,1.1]
    #scenario_elec_list = [0.7,1,1.3]
    ...
    for i in my_FTR_action_space:
        for item in i:
            if item[0] < 0:
                item[0] = -999
            if gv.policy == "hybrid":
                if item[2] not in [y.uid for y in Rowplayer.plants_owned] and item[2] != 4 and item[0] > 0:
                    item[0] = -999
                if abs(item[0]) < 0.01:
                    item[0] = -999
    ...
    #my_FTR_action_space = [[[5.38, 0.01, 0, 1], [9.63, 0.01, 0, 4], [-999, 0.01, 1, 0], [3.7, 0.01, 1, 4], [-999, 0.01, 4, 0], [-999, 0.01, 4, 1]]]
    #their_FTR_action_space = [[[3.7, 0.01, 0, 1], [7.4, 0.01, 0, 4], [-999, 0.01, 1, 0], [5.38, 0.01, 1, 4], [-999, 0.01, 4, 0], [-999, 0.01, 4, 1]]]
    #my_electricity_action_space = [(0.7,0.7)]
    #their_electricity_action_space = [(1.3,1.3)]
